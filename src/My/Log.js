...
function Admin(){
  ...
  const  [uyf, setFj] =useState('');
  const  [email, setEmail] =useState('');
  const  [password, setPassword] =useState('');
  const navigate = useNavigate();

  useEffect(()=>{
    {isAuthenticated && setEmail(user.email)}
    // {isAuthenticated ? (
    //   setFj(user.email)
    // ): 
...
const response = await fetch ('http://16.171.153.71:3306/userlog',config)
            ...
            <div class="col-md-6 col-lg-7 d-flex align-items-center">
              <div class="card-body p-4 p-lg-5 text-black">
                ...
                <form>
                  ...
                  {/* <button onClick={() => loginWithRedirect()} class="loginBtn loginBtn--google">
              Login with Google
            </button> */}
{/* 
