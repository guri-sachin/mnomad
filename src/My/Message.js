...
function Admin(){
  ...
  const navigate = useNavigate();
   useEffect(()=>{
    axios.get("http://16.171.153.71:3306/listmessage").then(res=>setProducts(res.data));
},[]);
 ...
 function Piya(e){
  ...
  axios.get(`http://16.171.153.71:3306/messagedelete/${id}`).then(
...
function onEdit(e){
  const id = e.target.id;
  axios.get(`http://16.171.153.71:3306/messageedit/${id}`).then(
      res =>{
     var diojk =res.data 
