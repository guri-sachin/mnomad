...
function Admin(){
  ...
  const [status, setStatus] = useState('');
   ...
   useEffect(()=>{
    axios.get("http://16.171.172.126:3306/subcategore").then(res=>setProducts(res.data));
},[product]);
...
function Show3(e){
  ...
  axios.post(`http://16.171.172.126:3306/subupdate/${id}`,data).then(
    res=> console.log(res))
    console.log(id);
  console.log(data);
  Swal.fire({
 ...
 function Piya(e){
  ...
  axios.get(`http://16.171.172.126:3306/subcatedelete/${id}`).then(
...
function onEdit(e){
  const id = e.target.id;
  axios.get(`http://16.171.172.126:3306/subediton/${id}`).then(
      res =>{
     var diojk =res.data 
    navigate('/Editsubcate', { state: diojk})
...
<div class="card-body"  align="right">
                  ...
                  <a href="add2"><button type="button" class="btn btn-outline-primary mb-1">ADD</button></a>
...
<div class="dropdown" style={{marginTop:"20px"}}>
  <button class="btn btn-outline-primary mb-1 dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"  onChange={handelDemo14}>
   {item.status}
  </button>
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
 
  <a  name={"Active"} onClick={Show3}  id={item.id}   class="dropdown-item">Active</a>
    <a  name={"Deactive"} onClick={Show3} id={item.id}  class="dropdown-item">Deactive</a>

    
    
  </div>
</div> </td>                        
                        <td><i class="fa fa-edit" id={item.id}  onClick={onEdit}></i></td>                        
                        <td><a href=""><i class="fa fa-trash"  id={item.id} onClick={Piya}></i></a></td>
                        </tr>
                 );
             })}
