...
function Admin(){
  ...
  const [status, setStatus] = useState('');
   ...
   useEffect(()=>{
    axios.get("http://16.171.153.71:3306/subcategore").then(res=>setProducts(res.data));
},[product]);
...
function Show3(e){
  ...
  axios.post(`http://16.171.153.71:3306/subupdate/${id}`,data).then(
    res=> console.log(res))
    console.log(id);
  console.log(data);
  Swal.fire({
 ...
 function Piya(e){
  ...
  axios.get(`http://16.171.153.71:3306/subcatedelete/${id}`).then(
...
function onEdit(e){
  const id = e.target.id;
  axios.get(`http://16.171.153.71:3306/subediton/${id}`).then(
      res =>{
     var diojk =res.data 
