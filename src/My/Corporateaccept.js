...
function Admin(){
  ...
  useEffect(()=>{
    ...
    // axios.get("http://localhost:4200/listcorpratein").then(res=>setProducts(res.data));
},[]);
                 ...
                 return(
                     <tr key={index}>
                      <td>{item.Fname} {item.Lname}</td>                    
                                                                           ...
                                                                           <td>{item.Rid}</td>
                        <td>          
                  <button type="button" class="btn btn-outline-primary mb-1" value={item.email} onClick={Subt}>ADD INVOICE</button> </td>
</tr> 
                 );
             })}
