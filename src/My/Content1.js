...
function Admin(){
  ...
  useEffect(()=>{
    axios.get("http://16.171.172.126:3306/contentlist").then(res=>setProducts(res.data));
},[]);
...
<div class="card-body" style={{marginLeft:"0", paddingLeft:"0", align:"right"}}>
                  ...
                  <a href="add6"><button type="button" class="btn btn-outline-primary mb-1">ADD CONTENT</button></a>
                     ...
                     <tr key={index}>
                        ...
                        <td>{item.sub}</td> 
                         <td>{item.room}</td> 
                          <td>{item.title}</td>                        
                        <td>{item.dsc}.</td>                        
                        <td>{item.link.length> 0 ? item.link:<video controls style={{height:"100px",width:"100px"}}>
