...
function Admin(){
  ...
  const [product,setProducts] =useState([]);
   useEffect(()=>{
    axios.post("http://16.171.172.126:3306/register").then(res=>setProducts(res.data));
},[]);
 ...
 function Piya(e){
  ...
  axios.get(`http://16.171.172.126:3306/redelete/${id}`).then(
...
async function Details(e){
  ...
  console.log(e.target.id)
   ...
   const response = await fetch("http://16.171.172.126:3306/listdetails",config);
   
   const data1 = await response.json();
   navigate('/Details',{state:data1})
  console.log(data1)
                 ...
                 return(
                     <tr key={index}>
                      ...
                      <td>XB78{item.id}</td>
                        ...
                        <td>{item.email}</td>
                        <td>{item.phone}</td> 
                        <td><img src={item.pic} style={{height:"100px",width:"100px"}}/></td>     

                  
<td><a ><AiOutlineEye style={{height:"28px",width:"28px"}} id={item.email} onClick={Details}/></a></td>
                         <td>  <a href="" id={item.id} onClick={Piya} ><i id={item.id} onClick={Piya} class="fa fa-trash"></i></a></td>
